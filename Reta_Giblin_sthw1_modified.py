...
st.markdown("""
The 5 changes I made were:
- Changed from mark_point to mark_circle
- Added a tooltip to show the values of x & y at each point
- Changed the color of the points to red
- Changed the opacity of the circles to 0.4
- Changed the use_container_width from TRUE to FALSE
""")

scatter = alt.Chart(df).mark_circle(color="red", opacity=0.4).encode(
    x='x', y='y', tooltip=['x','y'])
...
#    alt.Chart(source).mark_geoshape(fill='Green', stroke='navy')
#).project(
#    'naturalEarth1'
#).properties(width=900, height=600).configure_view(stroke=None)


#st.markdown("""
#The 2 changes I made were:
#- Changed the size to make it bigger
#- Changed the continent color to green and the line color to navy
#"""
#)
...
sales = [70, 57, 52, 49, 37,  20, 87, 24, 50, 70, 84, 90]

data = {"month":month, "sales":sales}
df = pd.DataFrame(data)

sales_bar = alt.Chart(df, title="Sales by Month").mark_bar().encode(
    x = "month",
    y = "sales"
).properties(
    width=800,
